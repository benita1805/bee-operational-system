...
import { FloweringDB, INDIA_REGIONS, MONTHS } from "../../src/data/indiaFloweringData";

import { ScreenContainer } from "../../src/components/layout/ScreenContainer";

/* ✅ Proper type including computed field */
type Crop = {
    crop: string;
    flowering: string[];
    nectarFlow: string;
...
type RegionKey = keyof typeof FloweringDB;

export default function FloweringCalendarScreen() {
    const [region, setRegion] = useState<RegionKey>("TN");
        ...
        <View style={{ flex: 1 }}>
            ...
            <ScreenContainer>
                ...
                <View style={{ paddingHorizontal: 16, marginTop: 8 }}>
                    ...
                    <FilterRow
                        options={INDIA_REGIONS.map((r) => ({
                            label: r.name,
                            value: r.id,
                        }))}
                        selected={region}
                        onSelect={(v) => setRegion(v as RegionKey)}
                    />
                </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <Card style={item.active ? [styles.card, styles.activeCard] : styles.card}>
                            <View style={styles.row}>
                                <Text style={styles.crop}>{item.crop}</Text>
                                <Text
                                    style={[
                                        styles.badge,
                                        item.active ? styles.on : styles.off,
                                    ]}
                                >
                            ...
                            <Text style={styles.meta}>
                                Flowering: {item.flowering.join(", ")} • Nectar:{" "}
                                {item.nectarFlow} • Yield: {item.yieldPotential}
                            </Text>
